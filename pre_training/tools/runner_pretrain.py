...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data, curvature, dist, atom_type) in enumerate(train_dataloader):
            num_iter += 1
            ...
            elif dataset_name == 'ProteinPretraining':
                points = data.cuda()
            ...
            loss = base_model(points, curvature, dist, atom_type)
            try:
                loss.backward()
                # print("Using one GPU")
            except:
