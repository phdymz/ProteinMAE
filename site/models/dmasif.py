...
from .transformer_model import Transformer_seg
...
class dMaSIF(nn.Module):
    def __init__(self, args):
        ...
        if args.embedding_layer == "dMaSIF":
            ...
            self.conv = Transformer_seg(args, I, E)
            if args.ckpt is not None:
    ...
    def forward(self, xyz, normal, curvature, dist, atom_type):
        ...
        torch.cuda.synchronize(device=xyz.device)
        torch.cuda.reset_max_memory_allocated(device=xyz.device)
        ...
        # R_values["conv"] = soft_dimension(embedding)

        torch.cuda.synchronize(device=xyz.device)
        # memory_usage = torch.cuda.max_memory_allocated(device=xyz.device)
